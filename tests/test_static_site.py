...
def test_publish_static_site_links_followup_dashboard_when_present(tmp_path: Path) -> None:
    output_dir = tmp_path / "output"
    site_dir = tmp_path / "site"
    output_dir.mkdir()
    site_dir.mkdir()
    (output_dir / "daily_brief_2026-05-17.html").write_text("<html><body>Brief</body></html>", encoding="utf-8")
    (site_dir / "followups.html").write_text("<html><body>Follow-ups</body></html>", encoding="utf-8")

    index_path = publish_static_site(output_dir=output_dir, site_dir=site_dir)

    index_text = index_path.read_text(encoding="utf-8")
    assert 'href="followups.html"' in index_text
